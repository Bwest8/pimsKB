...
-   **[CTE Student Fact Template](cte-student-fact.mdx)** 
    - Submit only if LEA had reportable enrollees in approved reimbursable secondary CTE programs and/or registered adult affidavit programs.
-   **[CTE Student Industry Credential Template](cte-student-industry-credential.mdx)** 
    - Submit only if CTE (secondary and/or adult affidavit) program enrollees earned reportable industry certifications.
...
In addition to these CTE templates, all **required** data elements within the June 30 Student Snapshot Template and School Enrollment Template must also be submitted for secondary CTE students for an LEA. 

Be aware that there are Student Snapshot Template data elements that are conditionally required specifically for secondary CTE students. Those elements include the address fields (Fields 16 – 20) along with the STUDENT IS A SINGLE PARENT (Field 120) and OUT OF WORKFORCE INDIVIDUAL (Field 166).

Note the following grade levels (Field 10, CURRENT GRADE LEVEL, within Student Snapshot Data elements) are considered valid for secondary CTE students:
